...
var (
	tableStyles = map[string]table.Style{
		// Wiregost styles
		WiregostDefault.Name: WiregostDefault,
	...
	WiregostDefault = table.Style{
		Name: "WiregostDefault",
		Box: table.BoxStyle{
			BottomLeft:       " ",
			BottomRight:      " ",
			BottomSeparator:  " ",
...
func GetTableStyle(con *console.Client) table.Style {
	if con.Settings == nil {
	...
	return tableStyles[WiregostDefault.Name]
}

// GetTableWithBordersStyle - Get the table style with borders
func GetTableWithBordersStyle(con *console.Client) table.Style {
	if con.Settings == nil {
		con.Settings, _ = assets.LoadSettings()
	}
	value, ok := tableStyles[con.Settings.TableStyle]
	if !ok || con.Settings.TableStyle == WiregostDefault.Name {
...
func PaginateTable(tw table.Writer, skipPages int, overflow bool, interactive bool, con *console.Client) {
