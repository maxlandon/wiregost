...
var cmdImportCA = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		certs.SaveCertificateAuthority("operator", cert, key)
	},
}
...
var cmdExportCA = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		certs.SetupCAs()
		certificateData, privateKeyData, err := certs.GetCertificateAuthorityPEM("operator")
		if err != nil {
		...
		if !os.IsNotExist(err) && fi.IsDir() {
			filename := fmt.Sprintf("%s.ca", filepath.Base("operator"))
