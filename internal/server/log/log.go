...
func GetRootAppDir() string {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if err != nil {
			panic("Cannot write to Wiregost root dir")
		}
	}
	return dir
}
...
func rootLogger() *logrus.Logger {
	...
	jsonFilePath := filepath.Join(GetLogDir(), "wiregost.json")
...
func txtLogger() *logrus.Logger {
	...
	txtFilePath := filepath.Join(GetLogDir(), "wiregost.log")
...
func (hook *TxtHook) Fire(entry *logrus.Entry) error {
	...
	srcFile := "<no caller>"
	if entry.HasCaller() {
		wiregostIndex := strings.Index(entry.Caller.File, "wiregost")
