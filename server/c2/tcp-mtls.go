...
import (
	...
	pb "github.com/maxlandon/wiregost/protobuf/ghost"
	"github.com/maxlandon/wiregost/server/certs"
	"github.com/maxlandon/wiregost/server/core"
	serverHandlers "github.com/maxlandon/wiregost/server/handlers"
...
var (
	mtlsLog = log.ServerLogger("c2", "mtls")
)
...
func StartMutualTLSListener(bindIface string, port uint16) (net.Listener, error) {
	...
	go acceptGhostConnections(ln)
	return ln, nil
}

func acceptGhostConnections(ln net.Listener) {
	for {
		conn, err := ln.Accept()
		if err != nil {
		...
		go handleGhostConnection(conn)
	}
}

func handleGhostConnection(conn net.Conn) {
	...
	defer func() {
		mtlsLog.Debugf("Cleaning up for %s", ghost.Name)
	...
	go func() {
		handlers := serverHandlers.GetGhostHandlers()
		for {
			...
			if envelope.ID != 0 {
				ghost.RespMutex.RLock()
				if resp, ok := ghost.Resp[envelope.ID]; ok {
					resp <- envelope // Could deadlock, maybe want to investigate better solutions
				}
				ghost.RespMutex.RUnlock()
			} else if handler, ok := handlers[envelope.Type]; ok {
				go handler.(func(*core.Ghost, []byte))(ghost, envelope.Data)
			}
		}
	}()

	for envelope := range ghost.Send {
	...
	mtlsLog.Infof("Closing connection to ghost %s", ghost.Name)
}
...
func getServerTLSConfig(host string) *tls.Config {
	...
	ghostCACert, _, err := certs.GetCertificateAuthority(certs.GhostCA)
	if err != nil {
		mtlsLog.Fatalf("Failed to find ca type (%s)", certs.GhostCA)
	}
	ghostCACertPool := x509.NewCertPool()
	ghostCACertPool.AddCert(ghostCACert)
	...
	tlsConfig := &tls.Config{
		RootCAs:                  ghostCACertPool,
		ClientAuth:               tls.RequireAndVerifyClientCert,
		ClientCAs:                ghostCACertPool,
