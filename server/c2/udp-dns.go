...
var (
	dnsLog = log.ServerLogger("c2", "dns")
...
func dnsSessionEnvelope(domain string, fields []string) ([]string, error) {
	...
	if dnsSession, ok := (*dnsSessions)[sessionID]; ok {
		...
		checkin := time.Now()
		dnsSession.Ghost.LastCheckin = &checkin

		// Response Envelope or Handler
		handlers := wiregostHandlers.GetGhostHandlers()
		if envelope.ID != 0 {
			dnsSession.Ghost.RespMutex.Lock()
			defer dnsSession.Ghost.RespMutex.Unlock()
			if resp, ok := dnsSession.Ghost.Resp[envelope.ID]; ok {
				resp <- envelope
			}
		} else if handler, ok := handlers[envelope.Type]; ok {
			handler.(func(*core.Ghost, []byte))(dnsSession.Ghost, envelope.Data)
		}
		return []string{"0"}, nil
	}
...
func dnsSessionPoll(domain string, fields []string) ([]string, error) {
	...
	for !isDrained {
		select {
		case envelope := <-dnsSession.Ghost.Send:
