...
import (
	...
	ver "github.com/maxlandon/wiregost/client/version"
...
var (
	setupLog = log.ServerLogger("assets", "setup")
...
func SetupGoPath(goPathSrc string) error {
	...
	pbGoSrc, err := protobufBox.Find("ghost/ghost.pb.go")
	if err != nil {
		setupLog.Info("static asset not found: ghost.pb.go")
		return err
	}
	pbConstSrc, err := protobufBox.Find("ghost/constants.go")
	if err != nil {
		setupLog.Info("static asset not found: constants.go")
		return err
	}

	protobufDir := path.Join(goPathSrc, "github.com", "maxlandon", "wiregost", "protobuf", "ghost")
	os.MkdirAll(protobufDir, os.ModePerm)
	ioutil.WriteFile(path.Join(protobufDir, "constants.go"), pbGoSrc, 0644)

	ioutil.WriteFile(path.Join(protobufDir, "ghost.pb.go"), pbConstSrc, 0644)
