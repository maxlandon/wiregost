...
func PersonalDirFilesSetup() {
	...
	fmt.Println(tui.Yellow(tui.Dim("----------------------------------------------------------" +
		"--------------------------------------------------------------------------------------")))
...
func SetUserCreds() (err error) {
	...
	fmt.Println(tui.Bold("If you have just set up the server, enter the same username !"))
	fmt.Println()
...
func SetUpDefaultServer() error {
	...
	fmt.Println(tui.Blue("  1. ") + "Copy the Certificate with extension '.crt' from ~/.wiregost/server/certificates/ " +
		"to ~/.wiregost/client/certificates/ ")
	...
	fmt.Println(tui.Blue("          * ") + "The FQDN field with the " + tui.Yellow("Common Name ") + "or " + tui.Yellow("server FQDN") + " that you filled when")
	fmt.Println("            running the server setup file. This name can be empty, as long as")
	fmt.Println("            they are identical in the Certificate and the server.conf file).")
	fmt.Println()
	fmt.Println(tui.Dim("--------------------------------------------------------------------------------------"))
