...
func SyntaxHighlighter(input []rune) (line string) {
	...
	args := strings.Split(string(input), " ")        // The readline input as a []string
	last := trimSpaceLeft([]rune(args[len(args)-1])) // The last char list in input
...
func highlightSubCommand(input string, args []string, command *flags.Command) (line string, remain []string) {
	line = input + tui.BOLD + command.Name + tui.RESET + " "
	remain = args[1:]
	return
}
